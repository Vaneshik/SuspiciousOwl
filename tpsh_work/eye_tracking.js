...
function contouring(thresh, mid, img, end_points, right = false) {
    let cnts, _ = cv.findContours(thresh, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_NONE);
    try {
        cnts = max(cnts, key = cv.contourArea);
...
function get_face_detector(modelFile = null, configFile = null, quantized = false) {
    if (quantized) {
        ...
        if (modelFile == null) {
            let modelFile = "models/res10_300x300_ssd_iter_140000.caffemodel"
        }
        if (configFile == null) {
...
function find_faces(img, model) {
    ...
    let blob = cv.dnn.blobFromImage(cv.resize(img, (300, 300)), 1.0, (300, 300), (104.0, 177.0, 123.0))
...
cv.createTrackbar('threshold', 'image', 75, 255, nothing())
...
while (true) {
    ...
    for (let rect = 0; rect < rects; rect++) {
        ...
        var thresh = cv.threshold(eyes_gray, threshold, 255, cv.THRESH_BINARY);
    ...
    if (cv.waitKey(1) && 0xFF === (var i = ch.codePointAt(pos))) {
        break
    }
}
cap.release()
